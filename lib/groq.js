...
- "low_profile_firmness_bump": explain that going with the thinner Low
  Profile means a firmer band than they might expect, to keep the same
  support with less material underneath.
...
- "extra_cush_travel_car_conflict": Extra Cush doesn't travel/fit car seats
  as well - see the secondCushion object (if present) for whether a Low
  Profile is also being offered as an option, or whether it's not suitable
  at their level and Regular's portability is the fallback.
- "hardseatpain_over_travel_stayed_regular": they said hard surfaces bother
  ...
  cushions start firm and soften with use, so err toward enough support).
  See the secondCushion object - Low Profile is offered as a secondary option
  with a caveat that it has even less cushioning.
