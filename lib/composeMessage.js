...
function composeResultMessage(routedResult, groqResult) {
  const sections = [
    `<h3>Why We Recommend A ${escapeHtml(routedResult.product)}</h3>`,
...
function composeFallbackEmailSummary(routedResult) {
  return truncateAtWord(
    `We've recommended the ${routedResult.product}, ${routedResult.firmness} firmness, ` +
      `${routedResult.size}, ${routedResult.thickness}.`,
