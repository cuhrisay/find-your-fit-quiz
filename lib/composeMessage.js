...
/**
 * Break-in note + return policy, always shown - plus the $8 exchange note,
 * only when routing.js flagged the result as having a real tradeoff (a second
 * cushion suggested, or a preference/need that didn't get fully met). Fixed,
 * code-generated content - never left to the AI, so it's never accidentally
 * dropped for the majority of results that see it, nor accidentally added to
 * a straightforward result where it would only add hesitation.
...
function composePolicyBlock(routedResult) {
  ...
  if (routedResult.alwaysInclude.exchangeNote) {
    parts.push(`<p>${escapeHtml(routedResult.alwaysInclude.exchangeNote)}</p>`);
  }
  return parts.join('\n');
}
...
function composeFallbackMessage(routedResult) {
  const sections = [
    `<h3>Why We Recommend The ${escapeHtml(getProductDisplayName(routedResult.product))}</h3>`,
    `<p>Based on what you told us, we'd recommend the ${escapeHtml(getProductDisplayName(routedResult.product))}, ` +
      `${escapeHtml(routedResult.firmness)} firmness, ${escapeHtml(routedResult.size)}, ` +
      `${escapeHtml(routedResult.thickness)}.</p>`,
    composePolicyBlock(routedResult),
  ];
  sections.push(MORE_SUPPORT_BLOCK);
  return sections.join('\n');
}
...
// platform limit, confirmed by real-world truncation) - nowhere near enough
// for the full formatted message above. AIMSG gets this short plain-text
// teaser instead; the full detail lives on the results page. No HTML here -
// every character costs budget in a field this small, and Mailchimp merge
// fields don't render markup as formatting anyway.
...
/**
 * @param {{whyWeRecommend: string}} groqResult
 * @param {object} routedResult
 * @returns {string} short plain-text teaser for Mailchimp's AIMSG merge field
 */
function composeEmailSummary(groqResult, routedResult) {
  const suffix = composeAlsoConsiderSuffix(routedResult.secondCushion);
  const mainBudget = EMAIL_SUMMARY_CHAR_LIMIT - suffix.length;
  return truncateAtWord(groqResult.whyWeRecommend, mainBudget) + suffix;
}

/**
 * @param {object} routedResult
 * @returns {string} short plain-text teaser used when Groq fails entirely
 */
function composeFallbackEmailSummary(routedResult) {
  const suffix = composeAlsoConsiderSuffix(routedResult.secondCushion);
  const mainBudget = EMAIL_SUMMARY_CHAR_LIMIT - suffix.length;
  return (
    truncateAtWord(
      `We've recommended the ${getProductDisplayName(routedResult.product)}, ` +
