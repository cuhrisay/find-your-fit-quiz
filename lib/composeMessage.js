...
function composeResultMessage(routedResult, groqResult) {
  const sections = [
    `<h3>Why We Recommend A ${escapeHtml(getProductDisplayName(routedResult.product))}</h3>`,
 ...
 */
function composeEmailSummary(groqResult) {
  return truncateAtWord(groqResult.whyWeRecommend, EMAIL_SUMMARY_CHAR_LIMIT);
}
