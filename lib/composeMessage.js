...
function composeWhyText(routedResult, answers) {
  ...
  if (routedResult.product === 'Simple Soother') {
    ...
    return (
      ...
      'it costs less, and its center cutout takes the pressure off that central area.'
    );
  }
  // The diagnosis belongs in the "why" itself when one was named - naming the
  // pain location alone credits the weaker signal when the person just told
  // us the condition. The mechanism here is deliberately the honest,
  // universal one (relieve pressure -> pelvic floor relaxes -> relief),
  // never "pressure off exactly where it hurts" - that's wrong for referred
  // pain like hips/thighs/buttocks, where the cushion never directly touches
  // the sore spot (see the CLAIM SAFETY note on DIAGNOSIS_TEXT).
  const cond = firstDiagnosisLabel(answers);
  return (
    `Based on your ${pain}${cond ? ` and your ${cond}` : ''}, the Twin Cheeks ` +
    'Folding Cushion is your fit. Its two adjustable panels support you at ' +
    'the sit bones while the open center lifts the pressure off the pelvic ' +
    "area — which lets the pelvic floor relax, and that's usually where the " +
    'relief comes from.'
  );
}
...
/**
 * Condition-specific paragraph, one per named condition. Replaces the old
 * generic "that's not rare, we hear about it often" line, which failed
 * hardest exactly where it mattered most.
 *
 * CLAIM SAFETY (Chrisie, July 2026 - these are firm rules, not style):
 * - NEVER claim the cushion treats, cures, is "designed for", or was "made
 *   for" any condition. It relieves pressure - that's the ceiling of the
 *   claim. Describe what it mechanically does (relieves pressure / lets the
 *   pelvic floor relax) and let that speak for itself.
 * - NEVER say "#1" / "number one". PN, tailbone pain/coccydynia, male
 *   prostatitis, and vulvodynia are "one of the most common" things people
 *   come to us about (per Chrisie/Trudy) - say exactly that, no ranking.
 ...
 *   relaxation is what eases pain - including referred pain felt somewhere
 *   the cushion doesn't directly touch (hips, thighs, buttocks). So for
 *   indirect/referred pain, DON'T imply "pressure off exactly where it
 *   hurts" - explain the pelvic-floor path instead.
 *
 ...
 * Twin Cheeks), so those two use product-neutral wording; the rest can
 * reference the Twin Cheeks channel and panels.
 */
const DIAGNOSIS_TEXT = {
  pudendal_neuralgia:
    'Pudendal neuralgia is one of the most common things people come to us ' +
    'about. The open channel keeps your weight off the pudendal nerve ' +
    "pathway, so it isn't compressed while you sit.",
  coccydynia:
    'Tailbone pain — coccydynia — is one of the most common reasons people ' +
    'find us. The open channel lets your tailbone rest suspended, carrying ' +
    'no weight at all while you sit.',
  prostatitis_cpps:
    'A lot of the discomfort that comes with a prostatitis or CPPS diagnosis ' +
    'is tied to sitting pressure and pelvic floor tension. Taking that ' +
    'pressure off the area gives those muscles a chance to relax.',
  vulvodynia:
    'Vulvodynia is something we hear about often. The open center keeps ' +
    'direct contact and pressure off that sensitive tissue while you sit.',
  pelvic_floor_dysfunction:
    'With pelvic floor dysfunction, sitting all day keeps those muscles ' +
    'under constant pressure. Lifting your weight off the pelvic floor gives ' +
    'them a real chance to relax.',
  interstitial_cystitis:
    'With interstitial cystitis, pressure through the pelvic area can make ' +
    'everything flare. Keeping your weight off that whole central area while ' +
    'you sit takes one aggravator out of the day.',
  endometriosis:
    'Endometriosis pain often radiates through the pelvic region and out to ' +
    'the hips. Easing the sitting pressure lets the surrounding muscles ' +
    'relax, which can ease some of that referred ache.',
  vaginismus:
    'With vaginismus, keeping the pelvic floor calm matters. Carrying your ' +
    'weight at the sit bones instead of across the pelvic floor gives those ' +
  ...
  recovering_surgery_childbirth:
    ...
    'pressure off tender, healing tissue is what this cushion does, and good ' +
    'support now can make sitting a lot more comfortable while you heal.',
};
