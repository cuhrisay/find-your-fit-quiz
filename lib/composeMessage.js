 ...
 */
function composeWhyText(routedResult) {
  if (routedResult.product === 'Simple Soother') {
    ...
    return (
      'Because your pain is centrally located rather than under your tailbone ' +
      'or sit bones, the Simple Soother is the right starting point — it costs ' +
      'less, and its center cutout is designed for exactly this kind of pain.'
    );
  }
  return (
    "Based on where your pain is, your weight, and how you'll be using it, the " +
    'Twin Cheeks Folding Cushion is your fit. Its two adjustable panels support ' +
    'you while the open center channel keeps pressure off the areas that hurt.'
  );
}
...
const NOTE_TEXT = {
  travel_low_profile:
    ...
    'the Low Profile — it folds up smaller and carries easily.',
  low_profile_firmness_bump:
    'Low Profile cushions are thinner, so we recommend going up one firmness ' +
    "level to keep the same support — that's already built into the firmness " +
    'shown above.',
...
function composeResultMessage(routedResult, answers, aiResult) {
  const sections = [
    ...
    `<p>${escapeHtml(composeWhyText(routedResult))}</p>`,
  ];
...
function emailDriverClause(routedResult) {
  ...
  if (notes.includes('travel_low_profile')) {
    return ' — Low Profile packs small for travel, one firmness level up to keep the same support.';
  }
