...
const FIELD_MAP = {
  ...
  firmPreference: 'question_RRJ4DJ', // Screen 5 (the one-sided question was removed July 2026)
  ...
  // Consent checkbox screen, added August 2026, sits right after "Begin Quiz"
  // and before any health question. Verified against a real webhook delivery
  // 2026-09-14 (field renamed to "Privacy Consent" in Tally around the same
  // time - that's just the display label and doesn't affect this key).
  consentGiven: 'question_j7DDAJ',
};

// Email/first name are no longer asked in the main quiz flow (August 2026) -
// they're collected later, optionally, on the results page ("email me my
// results + 10% off"), handled entirely by api/quiz-capture-email.js. If you
// remove those Tally questions per docs/Tally_Form_Changes.md, nothing here
// needs to change - FIELD_MAP simply no longer references them.
//
// Country/state/province were dropped the same way (August 2026) - they were
// never actually wired into any Meta Pixel/ads integration (this codebase has
// none), so there was no live use being lost by removing them.
...
function parseTallyPayload(payload) {
  return {
    ...
    // Tally may represent a checked checkbox as boolean `true` or as a
    // resolved option string, depending on the block type used - Boolean()
    // handles either shape, and treats unanswered/missing as false.
    consentGiven: Boolean(getSingleValue(payload, 'consentGiven')),
  };
}
