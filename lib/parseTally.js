...
const VALUE_MAPS = {
  ...
  needsMoreSpace: {
    Yes: 'yes',
    No: 'no',
    // Merged from the old separate "Sometimes" / "Not sure" options, which
    // used to behave differently in routeSize (Sometimes could size up to
    // 18", Not sure quietly defaulted to 16" like No). Mapped to 'sometimes'
    // to keep the size-up behavior - erring toward the more generous size
    // when they're not sure, consistent with how this project handles
    // ambiguity elsewhere (see ASSUMPTION notes throughout routing.js).
    'Sometimes / depends': 'sometimes',
  },
  recentEvents: {
