...
// 12. Low Profile firmness: NO bump (per Trudy's direct answer, July 2026) -
// traveling at Medium Plus stays Medium Plus, same as Regular would be.
{
  ...
  check('Travel does NOT bump firmness - stays Medium Plus, same as Regular', r.firmness, 'Medium Plus');
  check(
    'No firmness-bump note (rule removed)',
    r.notes.thickness.includes('low_profile_firmness_bump'),
    false
  );
  check(
...
// primary (they qualify), at the SAME firmness Regular would be (no bump),
// but flagged that it won't be as soft/cushy as they want (less depth to
// give under them, regardless of firmness label)
{
  const r = routeCushion(
  ...
  check('Travel + soft preference -> firmness stays Medium Plus (no bump)', r.firmness, 'Medium Plus');
  check(
...
// Same tradeoff note for tall + driving + soft preference
{
  ...
  check(
    'Tall + driving + soft preference -> firmness stays Medium Plus (no bump)',
    r.firmness,
    'Medium Plus'
  );
  check(
...
// 18" never comes in Low Profile (confirmed by Trudy, July 2026). Someone who
// confidently needs 18" AND travels often used to get an impossible "18",
// Low Profile" primary result - now falls back to Regular, with a Low
// Profile 16" suggested as a second-cushion companion for the road.
{
  const r = routeCushion(
    baseAnswers({ weight: '151-169', needsMoreSpace: 'yes', useCase: ['traveling'] })
  );
  check('18" + travel -> size stays 18"', r.size, '18" (Larger Size)');
  check('18" + travel -> thickness falls back to Regular (LP not available in 18")', r.thickness, 'Regular 2"');
  check('18" + travel -> firmness unaffected (Medium Plus, no bump)', r.firmness, 'Medium Plus');
  check(
    '18" + travel -> 18in-specific unavailable note, not the band one',
    r.notes.thickness.includes('low_profile_unavailable_18in') &&
      !r.notes.thickness.includes('low_profile_unavailable_at_band'),
    true
  );
  check(
    '18" + travel (no wheelchair/recliner) -> Low Profile 16" suggested as companion',
    r.secondCushion && r.secondCushion.suggestion && r.secondCushion.suggestion.thickness,
    'Low Profile 1.5"'
  );
  check(
    '18" + travel (no wheelchair/recliner) -> general trigger name, not the recliner-specific one',
    r.secondCushion.trigger,
    'eighteen_plus_travel_lowprofile_also_consider'
  );
  check(
    '18" + travel -> companion suggestion firmness has no bump either',
    r.secondCushion.suggestion.firmnessRange,
    'Medium Plus'
  );
}

// 18" + tall driver (no travel) -> same fallback, but no second-cushion
// suggestion (the existing scope only offers a companion for travel, not
// for driving alone)
{
  const r = routeCushion(
    baseAnswers({ height: 'tall', needsMoreSpace: 'yes', useCase: ['car_truck'] })
  );
  check('18" + tall driver -> thickness falls back to Regular', r.thickness, 'Regular 2"');
  check(
    '18" + tall driver -> 18in-specific unavailable note',
    r.notes.thickness.includes('low_profile_unavailable_18in'),
    true
  );
  check('18" + tall driver (no travel) -> no second-cushion suggestion', r.secondCushion, null);
}
