...
// Same lookup as findSubmissionById, but returns the whole record (with its
// Airtable record ID) so recordOrderClick/recordEmailSubmitted can PATCH it.
async function findSubmissionRecord(submissionId) {
  const apiKey = process.env.AIRTABLE_API_KEY;
  const baseId = process.env.AIRTABLE_BASE_ID;
  const tableName = process.env.AIRTABLE_SUBMISSIONS_TABLE || 'Submissions';
...
/**
 * Funnel tracking (Sep 2026): marks that this person clicked the results
 * page's Order button. Only a timestamp and a running count are stored —
 * nothing identifying — so the privacy split above still holds.
 * @param {string} submissionId
 * @returns {Promise<boolean>} false if no submission matched
 */
async function recordOrderClick(submissionId) {
  const record = await findSubmissionRecord(submissionId);
  if (!record) return false;
  await updateSubmissionRecord(record.id, {
    'Order Clicked At': new Date().toISOString(),
    'Order Clicks': (Number(record.fields['Order Clicks']) || 0) + 1,
  });
  return true;
}

/**
 * Funnel tracking: marks WHEN this person submitted the email form. The
 * address itself only ever goes to Mailchimp (see the privacy note above).
 * @param {string} submissionId
 */
async function recordEmailSubmitted(submissionId) {
  const record = await findSubmissionRecord(submissionId);
  if (!record) return false;
  await updateSubmissionRecord(record.id, {
    'Email Submitted At': new Date().toISOString(),
  });
  return true;
}
