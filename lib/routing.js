...
function routeSecondCushion(answers, size, thickness, firmness, thicknessResult) {
  const useCase = answers.useCase || [];
  const wantsTravel = useCase.includes('traveling') || useCase.includes('public_places');
  const wantsCar = useCase.includes('car_truck');
  ...
  // Extra Cush primary + travel/car: don't silently swap to Low Profile -
  // flag the conflict instead (matches the note already pushed in
  // routeThickness for the AI's own context).
  if (extraCushPrimary && wantsCarTravelOrPublic) {
    return {
      ...
      note:
        ...
        "Profile wouldn't give you enough support here to make sense as a " +
        'pairing. Worth asking which use case matters more, or pairing it ' +
        'with a second, more portable cushion.',
    };
  }

  // Low Profile never comes in 18", and Chrisie's asked that it not be offered
  // to anyone routed to Extra Firm or Soft - they likely wouldn't find it
  // supportive enough, and suggesting it anyway risks over-recommending/
  // confusing them. (Chrisie is still confirming with Trudy whether Extra
  // Firm folks should ever get this at all - for now, this blocks it outright.)
  const lowProfileUnsuitable =
    size === '18" (Larger Size)' || firmness === 'Extra Firm' || firmness === 'Soft';

  if (lowProfileUnsuitable) {
    return null;
  }

  // OPEN ITEM: this condition is currently unreachable. Whenever wantsTravel
  // is true and Extra Cush isn't primary, the branch above in routeThickness
  // always sets the PRIMARY thickness to Low Profile directly - so
  // `thickness !== 'Low Profile 1.5"'` can never be true here at the same time
  // as wantsTravel. Flag to Chrisie/Trudy: should "Regular for home, Low
  // Profile for travel" ever surface as a companion suggestion, rather than
  // Low Profile simply becoming the primary pick whenever travel is one of
  // several use cases? Left in place rather than removed since the intent
  // (the note text below) still reflects a real business rule worth revisiting.
