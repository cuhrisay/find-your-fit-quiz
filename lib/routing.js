...
function routeThickness(answers, firmnessResult) {
  ...
  // Profile (via travel/tall-driving) - see routeSecondCushion, which offers
  // Low Profile as a secondary option in this specific case.
  let lowProfileAlsoConsiderDueToHardSeatPain = false;
  ...
  // present and the base band already lands on Firm or Extra Firm - this is
  // the most supportive option available at that weight, so it wins outright
  // regardless of travel/driving (per Chrisie: the goal is enough support so
  // they don't bottom out - err toward firmer/more supportive, since these
  // cushions start firm and soften with use).
  if (hasHardSeatPain && (baseBand === 'Firm' || baseBand === 'Extra Firm')) {
    ...
    // it should win over a mere compactness WANT. Per Chrisie: the goal is one
    // cushion that covers all needs when possible, only recommending a second
    // when needs genuinely oppose each other. Here they do: staying on
    // Regular (more support than Low Profile) is the primary, with Low
    // Profile offered as a secondary option since portability clearly matters
    // to them - never swap straight to Low Profile and drop the pain signal.
    thickness = 'Regular 2"';
    notes.push('hardseatpain_over_travel_stayed_regular');
    lowProfileAlsoConsiderDueToHardSeatPain = true;
  } else if (wantsTravel) {
  ...
  if (wantsCar && !extraCushPrimary && !lowProfileAlsoConsiderDueToHardSeatPain) {
  ...
  return {
    ...
    baseBand,
    extraCushPrimary,
    lowProfileAlsoConsiderDueToHardSeatPain,
  };
}
...
function routeSecondCushion(answers, size, firmness, thicknessResult) {
  ...
  const { hasHardSeatPain, baseBand, extraCushPrimary, lowProfileAlsoConsiderDueToHardSeatPain } =
    thicknessResult;

  // 18" (from a confident "yes" to needing more space) + wheelchair/recliner
  // + travel: a wheelchair travels with them, so there's no separate
  // "need something compact" scenario the way there is for a recliner user -
  // no secondary suggestion, just a note acknowledging the tension. A
  // recliner user genuinely needs something different at home vs. on the go,
  // so Low Profile is worth mentioning there, if their firmness still allows it.
  if (size === '18" (Larger Size)' && (usesWheelchair || usesReclinerCouch) && wantsTravel) {
    if (usesWheelchair) {
      return {
        ...
        note:
          ...
          "isn't necessary the way it might be otherwise - just keep in mind " +
          "the 18\" is built to stay in place, not for portability.",
      };
    }
    if (firmness !== 'Soft' && firmness !== 'Extra Firm') {
      return {
        trigger: 'recliner_plus_travel_lowprofile_also_consider',
        suggestion: {
        ...
        note:
          "The 18\" is great for your recliner at home, but it's not built " +
          'for travel - a Low Profile is worth considering for on the go.',
      };
    }
    return {
      trigger: 'recliner_plus_travel_tradeoff',
      suggestion: null,
      note:
        "The 18\" is great for your recliner at home, but it's not built " +
        "for travel, and a Low Profile wouldn't give you enough support at " +
        'your level to make sense as a pairing.',
    };
  }

  // Hard-seat pain kept the primary on Regular instead of dropping to Low
  // Profile (support NEED beats compactness WANT) - but they clearly said
  // portability matters, so mention Low Profile as a secondary option with an
  // explicit caveat about reduced support. This is the mirror image of the
  // Extra Cush "also consider" below (Regular primary + Low Profile
  // secondary, never Low-Profile-primary + Extra-Cush-secondary - per
  // Chrisie, if both are ever in play, land on the one-cushion-fits-most
  // choice (Regular) as primary instead).
  if (lowProfileAlsoConsiderDueToHardSeatPain) {
    return {
      trigger: 'lowprofile_also_consider_hardseatpain_caveat',
      suggestion: {
        size,
        thickness: 'Low Profile 1.5"',
        firmnessRange: LOW_PROFILE_FIRMNESS_RANGE,
      ...
      note:
        'Since portability matters to you, a Low Profile is an option here - ' +
        'just know it has even less cushioning than what we recommended, and ' +
        'given the hard-surface sensitivity you mentioned, it may not give ' +
        'you enough support. Worth deciding which matters more.',
    };
  }
...
function routeCushion(answers) {
  ...
  // Per Chrisie: only mention the $8 exchange option when the recommendation
  // involves a real tradeoff/second cushion - a second cushion suggestion, a
  // preference that didn't get what it wanted, or a compact-vs-support
  // conflict. Never show it on a straightforward result, to avoid adding
  // hesitation where there isn't any.
  const hasOpposingNeeds =
    secondCushion !== null ||
    thicknessResult.notes.includes('low_profile_soft_preference_tradeoff') ||
    thicknessResult.notes.includes('extra_cush_travel_car_conflict') ||
    thicknessResult.notes.includes('low_profile_unavailable_at_band');
    ...
    alwaysInclude: {
      ...
      eighteenInchExplanation:
        sizeResult.size === '18" (Larger Size)'
          ...
          : null,
      exchangeNote: hasOpposingNeeds
        ? "If you order this and it doesn't feel right, we offer exchanges for " +
          "just $8 to cover shipping. You've got 14 days to decide, and if you " +
          'need more time, just email us and let us know — no problem.'
        : null,
    },
  };
}
