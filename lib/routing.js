  ...
  if (size === '18" (Larger Size)' && wantsTravel) {
    ...
    if (firmness !== 'Soft' && firmness !== 'Extra Firm') {
      return {
        ...
        note:
          `The 18" is great for ${homeContext}, but it's not built for ` +
          'travel - a Low Profile is worth considering for on the go.',
      };
    }
    return {
      ...
      note:
        `The 18" is great for ${homeContext}, but it's not built for ` +
        "travel, and a Low Profile wouldn't give you enough support at your " +
        'level to make sense as a pairing.',
    };
  }
