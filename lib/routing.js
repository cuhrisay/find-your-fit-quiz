...
/**
 * The SPECIFIC firmness Low Profile would need at a given starting band - one
 * level up, capped at Firm (same bump rule as when Low Profile is actually
 * chosen as primary - see routeThickness). Used for "also consider" secondary
 * suggestions so they show one concrete firmness, not the whole Medium/Medium
 * Plus/Firm range Low Profile happens to be available in.
 * @param {string} startingBand - a FIRMNESS_BANDS value
 * @returns {string} a FIRMNESS_BANDS value
 */
function lowProfileFirmnessSuggestion(startingBand) {
  const idx = FIRMNESS_BANDS.indexOf(startingBand);
  const bumpedIdx = Math.min(idx + 1, 3); // capped at Firm - Low Profile doesn't come in Extra Firm
  return FIRMNESS_BANDS[bumpedIdx];
}
    ...
    if (firmness !== 'Soft' && firmness !== 'Extra Firm') {
      return {
        ...
        suggestion: {
          ...
          thickness: 'Low Profile 1.5"',
          firmnessRange: lowProfileFirmnessSuggestion(firmness),
        },
        note:
