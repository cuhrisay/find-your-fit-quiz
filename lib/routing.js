...
function routeThickness(answers, firmnessResult) {
  ...
  if (extraCushTriggered) {
    ...
    // locations/hardSeatPain (which drove product/firmness, not thickness) -
    // without this, the AI has nothing to point to and has been observed
    // inventing a false connection to hip pain instead.
  ...
  // on Low Profile, fall back to Regular 2" and note why.
  if (thickness === 'Low Profile 1.5"' && (firmnessIndex < 1 || firmnessIndex > 3)) {
    ...
    // ASSUMPTION: capped at Firm (index 3) rather than letting a Firm base band
    // bump into Extra Firm, since Low Profile doesn't physically come in Extra
    // Firm fabric. Chrisie is still confirming this exact edge case with Trudy -
    // flag before trusting this in production.
...
function routeSecondCushion(answers, size, thickness, firmness) {
  const useCase = answers.useCase || [];
  const wantsTravel = useCase.includes('traveling') || useCase.includes('public_places');
  const wantsHome = useCase.includes('one_chair') || useCase.includes('office_desk');

  // Low Profile never comes in 18", and Chrisie's asked that it not be offered
  // to anyone routed to Extra Firm, Soft, or an Extra Cush recommendation at a
  // heavier weight - they likely wouldn't find it supportive enough, and
  // suggesting it anyway risks over-recommending/confusing them. (Chrisie is
  // still confirming with Trudy whether Extra Firm/Extra Cush folks should ever
  // get this at all - for now, this blocks it outright in those cases.)
  // ASSUMPTION: "over 200 lbs" doesn't align with the weight buckets the quiz
  // collects (200 falls inside the 180-220 bucket) - treating the whole
  // 180-220 bucket as "over 200" here, erring toward NOT suggesting Low
  // Profile when in doubt, same reasoning as the Extra Cush weight threshold
  // above.
  const isOver200lb = answers.weight === '180-220' || answers.weight === 'over220';
  const lowProfileUnsuitable =
    size === '18" (Larger Size)' ||
    firmness === 'Extra Firm' ||
    firmness === 'Soft' ||
    (thickness === 'Extra Cush 3"' && isOver200lb);

  if (thickness === 'Extra Cush 3"' && wantsTravel) {
    if (lowProfileUnsuitable) {
      return {
        ...
        note:
          "Extra Cush doesn't fold well for travel, and a Low Profile wouldn't " +
          "give you enough support here to make sense as a pairing. Worth " +
          'asking which use case matters more, or pairing it with a second, ' +
          'more portable cushion.',
      };
    }
    return {
      trigger: 'extracush_plus_travel_lowprofile_note',
      suggestion: {
        size: '16" (Regular Size)',
        thickness: 'Low Profile 1.5"',
      ...
      note:
        "A Low Profile could work well for travel here — just keep in mind it " +
        "doesn't have the Extra Cush layer.",
    };
  }
  ...
  if (wantsHome && wantsTravel && thickness !== 'Low Profile 1.5"') {
    return {
      ...
      suggestion: {
        size, // guaranteed 16" here - 18" already returned null above
        thickness: 'Low Profile 1.5"',
        firmnessRange: LOW_PROFILE_FIRMNESS_RANGE,
      },
